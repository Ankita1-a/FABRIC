...
class LMMDLoss(MMDLoss, LambdaSheduler):
    ...
    def forward(self, source, target, source_label, target_logits):
        if self.kernel_type == 'linear':
            ...
            weight_ss = torch.from_numpy(weight_ss).to(my_device)  # B, B
            weight_tt = torch.from_numpy(weight_tt).to(my_device)
            weight_st = torch.from_numpy(weight_st).to(my_device)
