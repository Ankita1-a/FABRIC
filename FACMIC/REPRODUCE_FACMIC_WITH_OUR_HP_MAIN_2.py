        ...
        with torch.no_grad():
            ...
            for client_idx, model in enumerate(models):
                if client_idx in args.test_envs:
                    ...
                    print(train_acc)
                    print(f'Site-{client_idx}| Train Acc: {train_acc}')
                    logrecord += f'Site-{client_idx}| Train Acc: {train_acc}\n'

                    val_acc = test(
                        args, model, val_loaders[client_idx], device)
                    val_acc_list[client_idx] = val_acc
                    print(f'Site-{client_idx}| Val  Acc: {val_acc}', flush=True)
                    logrecord += f'Site-{client_idx}| Val  Acc: {val_acc}\n'
            # best_acc = 0
            ...
            for client_idx in range(client_num):
                ...
                print(f'Test site-{client_idx}| Test Acc: {test_acc} | Bacc: {bacc}')
                logrecord += f'Test site-{client_idx}| Test Acc: {test_acc}'
                test_acc_list[client_idx] = test_acc

    np_log = np.array(log, dtype=float)
    np.savetxt('./results/Global_Acc7_new.csv', np_log, delimiter=',', fmt='%.6f')
    np_log2 = np.array(log2, dtype=float)
    np.savetxt('./results/Global_bacc7_new.csv', np_log2, delimiter=',', fmt='%.6f')