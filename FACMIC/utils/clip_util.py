import clip
import torch
from utils.config import my_device
...
def get_image_features(image, model, cpreprocess, device=my_device, need_preprocess=False):
    if need_preprocess:
        print("aaa")
...
def get_text_features_list(texts, model, device=my_device, train=False):
    if train:
